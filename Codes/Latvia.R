...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/latvia.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #0.3023397
(sum(hicp$var) / sum(intr$var)) #-0.2638847

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #0.3065221
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.2598835

(sum(gdp$var[1]) / sum(intr$var[1])) #0.4637981
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.2710759
...
quantile(gdp_intr, probs = lower, na.rm = T) #0.1859022                
quantile(gdp_intr, probs = upper, na.rm = T) #0.3981265                 

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.00346475

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4406342                  
quantile(hicp_intr, probs = upper, na.rm = T) #-0.09781052                  

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005588501
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #0.1928533                 
quantile(gdp_intr, probs = upper, na.rm = T) #0.4007792                   

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.003365051

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4404114                   
quantile(hicp_intr, probs = upper, na.rm = T) #-0.09569386                   

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005540378
...
quantile(gdp_intr, probs = lower, na.rm = T) #0.3582473                 
quantile(gdp_intr, probs = upper, na.rm = T) #0.5692426                
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.003383215

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.5342203                  
quantile(hicp_intr, probs = upper, na.rm = T) #0.0266279                  
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.008205757
