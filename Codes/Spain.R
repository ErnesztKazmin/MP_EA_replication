...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/spain.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #-0.8775916
(sum(hicp$var) / sum(intr$var)) #-0.4907794

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.528801
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.4777094

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.2408903
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.3751677
...
quantile(gdp_intr, probs = lower, na.rm = T) #-2.267453              
quantile(gdp_intr, probs = upper, na.rm = T) #-0.1756753                

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.8543174

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.8764845                
quantile(hicp_intr, probs = upper, na.rm = T) #-0.2670039                

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.1577976
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-1.12191              
quantile(gdp_intr, probs = upper, na.rm = T) #-0.1752944                

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.02044681

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.7522443                 
quantile(hicp_intr, probs = upper, na.rm = T) #-0.2846496                

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.009808496
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.377234               
quantile(gdp_intr, probs = upper, na.rm = T) #-0.1308587              
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.004161705

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.5437575                
quantile(hicp_intr, probs = upper, na.rm = T) #-0.2318233                
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005051564
