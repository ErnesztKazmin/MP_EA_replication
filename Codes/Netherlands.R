...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/netherlands.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #-0.07541469
(sum(hicp$var) / sum(intr$var)) #-0.131814

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.157455
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.1447852

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.325541
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.1412102
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.5672796                   
quantile(gdp_intr, probs = upper, na.rm = T) #0.1913475                    

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.09845355

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.2958105                     
quantile(hicp_intr, probs = upper, na.rm = T) #-0.03458364                     

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.01304634
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.5101423                       
quantile(gdp_intr, probs = upper, na.rm = T) #0.06913833                       

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01143331

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.3010926                      
quantile(hicp_intr, probs = upper, na.rm = T) #-0.04361585                      

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.004728085
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.4861422                    
quantile(gdp_intr, probs = upper, na.rm = T) #-0.1850529                   
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.005023401

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.3158059                     
quantile(hicp_intr, probs = upper, na.rm = T) #0.009787749                     
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005322886
