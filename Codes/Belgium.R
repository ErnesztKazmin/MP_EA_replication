...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/belgium.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #-0.3770808
(sum(hicp$var) / sum(intr$var)) #-0.3289614

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.3065532
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.3093492

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.1255535
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.2016342
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.8827006         
quantile(gdp_intr, probs = upper, na.rm = T) #-0.07370958          

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.1242744

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.6057047           
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1472583           

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.05589091
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.6756055          
quantile(gdp_intr, probs = upper, na.rm = T) #-0.0695181           

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01365372

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.5441475            
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1450671            

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.008296987
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.2482725          
quantile(gdp_intr, probs = upper, na.rm = T) #-0.008530942         
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.003970664

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4270743           
quantile(hicp_intr, probs = upper, na.rm = T) #0.006057546           
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.007043118
