...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/slovenia.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #-0.0827928
(sum(hicp$var) / sum(intr$var)) #-0.2427919

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.1751695
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.2394439

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.5715336
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.2272305
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.9106854             
quantile(gdp_intr, probs = upper, na.rm = T) #0.3800478               

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.02808264

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4617497               
quantile(hicp_intr, probs = upper, na.rm = T) #-0.09432237               

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.007355037
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.8982351              
quantile(gdp_intr, probs = upper, na.rm = T) #0.2626302               

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.02241797

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4620242                
quantile(hicp_intr, probs = upper, na.rm = T) #-0.08041378                

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.007168709
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.9707759              
quantile(gdp_intr, probs = upper, na.rm = T) #-0.2648543             
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01153116

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.5251634               
quantile(hicp_intr, probs = upper, na.rm = T) #0.03192412               
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.009559649