...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/italy.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #-0.9549554
(sum(hicp$var) / sum(intr$var)) #-0.154548

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.7372183
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.1352729

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.4743178
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.06335413
...
quantile(gdp_intr, probs = lower, na.rm = T) #-2.195832               
quantile(gdp_intr, probs = upper, na.rm = T) #-0.2609958                

sd(gdp_intr)/sqrt(length(gdp_intr)) #1.06278

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.3352745                 
quantile(hicp_intr, probs = upper, na.rm = T) #-0.04154245                 

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.1959305
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-1.479123                
quantile(gdp_intr, probs = upper, na.rm = T) #-0.3094596                 

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.03018321

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.264071                  
quantile(hicp_intr, probs = upper, na.rm = T) #-0.05176559                  

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005012716
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.661012                
quantile(gdp_intr, probs = upper, na.rm = T) #-0.3091868               
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.005687575

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.1444333                 
quantile(hicp_intr, probs = upper, na.rm = T) #0.009239002                 
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.002509549
