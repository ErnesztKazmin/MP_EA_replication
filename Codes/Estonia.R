...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/estonia.xlsx")
...
#tseries::adf.test(data$gdp_f) # Not stationary
#tseries::kpss.test(data$gdp_f) # Stationary
...
(sum(gdp$var) / sum(intr$var)) #0.2280836
(sum(hicp$var) / sum(intr$var)) #-0.2847356

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #0.2305351
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.283912

(sum(gdp$var[1]) / sum(intr$var[1])) #0.3047837
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.2821614
...
quantile(gdp_intr, probs = lower, na.rm = T) #0.05162484          
quantile(gdp_intr, probs = upper, na.rm = T) #0.4492205           

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.006469327

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4855825            
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1117024            

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005797383
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #0.05844615           
quantile(gdp_intr, probs = upper, na.rm = T) #0.4447625            

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.006371008

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4858944             
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1102648             

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005848227
...
quantile(gdp_intr, probs = lower, na.rm = T) #0.1003164          
quantile(gdp_intr, probs = upper, na.rm = T) #0.5373709          
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.006804168

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.5452971            
quantile(hicp_intr, probs = upper, na.rm = T) #-0.05168136            
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.008268659
