...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/luxembourg.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #-0.8805656
(sum(hicp$var) / sum(intr$var)) #-0.5088934

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.8898019
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.5157914

(sum(gdp$var[1]) / sum(intr$var[1])) #-1.110379
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.4547164
...
quantile(gdp_intr, probs = lower, na.rm = T) #-1.608296                 
quantile(gdp_intr, probs = upper, na.rm = T) #-0.4465831                  

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.02050281

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.800946                   
quantile(hicp_intr, probs = upper, na.rm = T) #-0.3228557                   

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.008906458
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-1.600629                   
quantile(gdp_intr, probs = upper, na.rm = T) #-0.4759761                    

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01985539

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.8038613                    
quantile(hicp_intr, probs = upper, na.rm = T) #-0.3345567                    

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.008789556
...
quantile(gdp_intr, probs = lower, na.rm = T) #-1.594026                  
quantile(gdp_intr, probs = upper, na.rm = T) #-0.7032017                 
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01451679

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.8315651                   
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1212119                   
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.01132392
