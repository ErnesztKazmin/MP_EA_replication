...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/france.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #-0.05565644
(sum(hicp$var) / sum(intr$var)) #-0.2039044

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.1132962
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.2185604

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.4856894
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.1248135
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.416433            
quantile(gdp_intr, probs = upper, na.rm = T) #0.1407566             

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01391135

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.3676891              
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1038235              

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005307996
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.448228             
quantile(gdp_intr, probs = upper, na.rm = T) #0.07303764              

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01675874

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.3883884               
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1123943               

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.006715806
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.68151             
quantile(gdp_intr, probs = upper, na.rm = T) #-0.3239443            
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.00629034

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.3207569              
quantile(hicp_intr, probs = upper, na.rm = T) #0.0384641             
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005771473