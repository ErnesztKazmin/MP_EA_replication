...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/portugal.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #0.01014572
(sum(hicp$var) / sum(intr$var)) #-0.2660508

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.06662726
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.2652999

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.4142261
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.6752964
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.3828329           
quantile(gdp_intr, probs = upper, na.rm = T) #0.2410653            

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01072674

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4609964             
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1388594             

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005645247
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.4334251            
quantile(gdp_intr, probs = upper, na.rm = T) #0.1568909             

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.009974513

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4547151              
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1375363              

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005478808
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.6660365            
quantile(gdp_intr, probs = upper, na.rm = T) #-0.213065           
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.007306748

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.9402923             
quantile(hicp_intr, probs = upper, na.rm = T) #-0.4683582             
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.008143204
