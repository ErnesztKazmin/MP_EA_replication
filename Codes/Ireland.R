...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/ireland.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #-0.6882152
(sum(hicp$var) / sum(intr$var)) #-0.2734826

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.6966704
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.2760658

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.9528524
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.3104349
...
quantile(gdp_intr, probs = lower, na.rm = T) #-1.587095              
quantile(gdp_intr, probs = upper, na.rm = T) #-0.03309554               

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.02842937

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4600181                
quantile(hicp_intr, probs = upper, na.rm = T) #-0.144                

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005866282
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-1.598438               
quantile(gdp_intr, probs = upper, na.rm = T) #-0.04743906                

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.02819799

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4591217                  
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1495657                 

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005812026
...
quantile(gdp_intr, probs = lower, na.rm = T) #-1.89495               
quantile(gdp_intr, probs = upper, na.rm = T) #-0.1574117              
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.03073127

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4834559                
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1632788                
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005306881