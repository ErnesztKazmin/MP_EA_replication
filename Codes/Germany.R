...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/germany.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #-0.2585766
(sum(hicp$var) / sum(intr$var)) #-0.123536

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.3009855
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.1270575

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.7410613
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.1069541
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.807551             
quantile(gdp_intr, probs = upper, na.rm = T) #0.06815536              

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.04968409

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.2133295               
quantile(hicp_intr, probs = upper, na.rm = T) #-0.0722398               

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.007432724
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.7709011              
quantile(gdp_intr, probs = upper, na.rm = T) #0.004701018               

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01564868

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.2079319                
quantile(hicp_intr, probs = upper, na.rm = T) #-0.07739298                

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.002543867
...
quantile(gdp_intr, probs = lower, na.rm = T) #-1.013138              
quantile(gdp_intr, probs = upper, na.rm = T) #-0.5038697             
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.008449857

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.1641384               
quantile(hicp_intr, probs = upper, na.rm = T) #-0.06097955               
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.001685071