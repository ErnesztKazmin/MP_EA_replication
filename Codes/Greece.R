...
# Data Preparation --------------------------------------------------------
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/greece.xlsx")
...
tseries::kpss.test(data$dlgdp) # Not stationary
...
(sum(gdp$var) / sum(intr$var)) #0.02282691
(sum(hicp$var) / sum(intr$var)) #-0.1572756

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.06433503
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.1597761

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.6415106
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.2586785
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.5506527              
quantile(gdp_intr, probs = upper, na.rm = T) #0.559985               

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01988466

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.2829511                
quantile(hicp_intr, probs = upper, na.rm = T) #-0.05942501                

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.00389406
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.5928425               
quantile(gdp_intr, probs = upper, na.rm = T) #0.4437253                

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01858272

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.2842009                 
quantile(hicp_intr, probs = upper, na.rm = T) #-0.06056178                 

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.003836993
...
quantile(gdp_intr, probs = lower, na.rm = T) #-1.129739               
quantile(gdp_intr, probs = upper, na.rm = T) #-0.1936884              
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01498216

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.446732                
quantile(hicp_intr, probs = upper, na.rm = T) #-0.09305437                
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.0060308