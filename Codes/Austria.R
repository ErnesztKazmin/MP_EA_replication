...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/austria.xlsx")
data$date <- as.Date(data$date, tryFormats = "%Y-%m-%d")
data$shadow_d <- as.numeric(data$shadow_d)
...
tseries::adf.test(data$dlgdp) # Not stationary
...
tseries::adf.test(data$shadow_d) # Stationary
tseries::kpss.test(data$shadow_d) # Stationary
...
(sum(gdp$var) / sum(intr$var)) #0.03058451
(sum(hicp$var) / sum(intr$var)) #-0.2706183

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #0.02081665
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.2746613

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.03425048
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.2707637
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.329082        
quantile(gdp_intr, probs = upper, na.rm = T) #0.2555313         

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01245279

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4659682          
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1367523          

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.00671107
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.3096313         
quantile(gdp_intr, probs = upper, na.rm = T) #0.2340289          

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01026147

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.4617615           
quantile(hicp_intr, probs = upper, na.rm = T) #-0.1437656           

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005891181
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.2210539         
quantile(gdp_intr, probs = upper, na.rm = T) #0.1339063      
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.005615855

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.487159          
quantile(hicp_intr, probs = upper, na.rm = T) #-0.08159719          
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.006344071
