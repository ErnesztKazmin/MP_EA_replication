...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/malta.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #0.333464
(sum(hicp$var) / sum(intr$var)) #0.09084349

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #0.2912276
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #0.08725455

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.229312
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.2579466
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.1672542                  
quantile(gdp_intr, probs = upper, na.rm = T) #0.7566058                   

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01546246

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.05859037                    
quantile(hicp_intr, probs = upper, na.rm = T) #0.2583315                    

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.005017789
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.2056803                    
quantile(gdp_intr, probs = upper, na.rm = T) #0.6927058                     

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01520826

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.06103339                     
quantile(hicp_intr, probs = upper, na.rm = T) #0.2560125                     

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.004985381
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.7609113                   
quantile(gdp_intr, probs = upper, na.rm = T) #0.242215                  
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.0170843

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.5016424                    
quantile(hicp_intr, probs = upper, na.rm = T) #0.01005452                    
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.008124023
