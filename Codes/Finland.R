...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/finland.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #0.3108991
(sum(hicp$var) / sum(intr$var)) #-0.1215415

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #0.2155676
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.129399

(sum(gdp$var[1]) / sum(intr$var[1])) #-0.6220783
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.2038314
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.1660932           
quantile(gdp_intr, probs = upper, na.rm = T) #0.6239862            

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01426584

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.2387309             
quantile(hicp_intr, probs = upper, na.rm = T) #-0.04801465             

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.003393941
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.2345693            
quantile(gdp_intr, probs = upper, na.rm = T) #0.5155205             

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01365501

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.2522521              
quantile(hicp_intr, probs = upper, na.rm = T) #-0.05264443              

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.003415077
...
quantile(gdp_intr, probs = lower, na.rm = T) #-1.016576            
quantile(gdp_intr, probs = upper, na.rm = T) #-0.2900273           
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01243781

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.3720149             
quantile(hicp_intr, probs = upper, na.rm = T) #-0.08720872             
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.004698842
