...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/cyprus.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #-0.4375333
(sum(hicp$var) / sum(intr$var)) #-0.2211427

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #-0.219233
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.2076075

(sum(gdp$var[1]) / sum(intr$var[1])) #0.04444543
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.1727834
...
quantile(gdp_intr, probs = lower, na.rm = T) #-1.312475           
quantile(gdp_intr, probs = upper, na.rm = T) #0.3302087            

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.05257729

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.5943946             
quantile(hicp_intr, probs = upper, na.rm = T) #0.02328075             

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.01312905
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.8308123            
quantile(gdp_intr, probs = upper, na.rm = T) #0.2688174             

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.01895045

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.5453135              
quantile(hicp_intr, probs = upper, na.rm = T) #0.0158765              

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.009347027
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.23779            
quantile(gdp_intr, probs = upper, na.rm = T) #0.3099815           
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.009050332

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.5976338             
quantile(hicp_intr, probs = upper, na.rm = T) #0.1951584             
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.01256489
