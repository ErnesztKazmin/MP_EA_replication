...
data <- readxl::read_xlsx("C:/Users/ernes/Desktop/MP Data/Replication/Final Data/lithuania.xlsx")
...
(sum(gdp$var) / sum(intr$var)) #0.06971968
(sum(hicp$var) / sum(intr$var)) #-0.197377

(sum(gdp$var[1:4]) / sum(intr$var[1:4])) #0.07483747
(sum(hicp$var[1:4]) / sum(intr$var[1:4])) #-0.1949277

(sum(gdp$var[1]) / sum(intr$var[1])) #0.2005988
(sum(hicp$var[1]) / sum(intr$var[1])) #-0.1059967
...
quantile(gdp_intr, probs = lower, na.rm = T) #-0.1053428                
quantile(gdp_intr, probs = upper, na.rm = T) #0.2434653                 

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.00552771

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.3661376                  
quantile(hicp_intr, probs = upper, na.rm = T) #-0.0613215                  

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.004915682
...
gdp_intr <- sum_gdp / sum_intr
quantile(gdp_intr, probs = lower, na.rm = T) #-0.09118438                 
quantile(gdp_intr, probs = upper, na.rm = T) #0.2431998                  

sd(gdp_intr)/sqrt(length(gdp_intr)) #0.005314082

hicp_intr <- sum_hicp / sum_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.3600598                   
quantile(hicp_intr, probs = upper, na.rm = T) #-0.06027969                    

sd(hicp_intr)/sqrt(length(hicp_intr)) #0.004898111
...
quantile(gdp_intr, probs = lower, na.rm = T) #0.0471775                 
quantile(gdp_intr, probs = upper, na.rm = T) #0.3665947                
sd(gdp_intr)/sqrt(length(gdp_intr)) #0.005008803

hicp_intr <- impact_hicp / impact_intr
quantile(hicp_intr, probs = lower, na.rm = T) #-0.3486951                  
quantile(hicp_intr, probs = upper, na.rm = T) #0.1146046                  
sd(hicp_intr)/sqrt(length(hicp_intr)) #0.007015899
